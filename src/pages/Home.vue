<template>
  <div>
    ...
    <div class="space-y-8" v-else>
         <!-- Cards -->
        <div class="max-w-xl">
          <h1 class="mt-20 text-4xl leading-10 sm:text-5xl font-extrabold tracking-tight text-slate-50">Your Gateway to Irresistible TV Delights</h1>
          <p class="mt-4 max-w-3xl space-y-6 text-gray-400">Unlock your TV obsession with captivating dramas and hilarious comedies, all at your fingertips</p>
        </div>
        ...
        <div v-for="(genre,index) in genres" :key="index" >
          <div class="text-2xl tracking-tight font-bold text-slate-200 mb-3 p-2 flex items-center space-x-2">
            <div>{{genre.emoji}}</div>
            <div>{{genre.name}}</div>
          </div>
